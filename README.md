...
SOOP의 현재 방송 상태 API와 별도로, 사이트에 운영자가 입력한 **다음 방송 예정 시간**을 표시할 수 있습니다.

`config.js`의 `soopSchedule`에 KST 기준 ISO 8601 시간을 입력하면 현재 시각 이후의 가장 가까운 일정이 자동으로 선택됩니다.

예:
```js
soopSchedule: [
  {
    startAt: "2026-10-03T20:00:00+09:00",
    title: "3000일 기념 방송"
  }
]
```

- 일정은 브라우저에서 30초마다 갱신됩니다.
- 일정이 지나면 다음 미래 일정으로 자동 전환됩니다.
- `url`을 추가하면 해당 일정의 버튼 링크도 지정할 수 있습니다.
- SOOP의 현재 공개 라이브 API는 방송 중 상태/상세 정보 조회를 제공하지만, 공개 문서로 확인되는 형태의 개인 스트리머별 '다음 예약 방송' API는 확인되지 않았습니다. 따라서 이 기능은 운영자가 실제 예정 시간을 `config.js`에 등록하는 방식으로 구현했습니다. 비공식 SOOP API 라이브러리들도 현재 라이브 상태/상세 조회를 중심으로 제공하고 있습니다. 
