...
-- =========================================
-- SOOP VIEWER STATISTICS / BROWSER COLLECTION
-- =========================================
-- Vercel Cron을 사용하지 않고, 방문자가 웹사이트를 열어 둔 동안
-- 브라우저가 방송 중 시청자 수를 1분 단위로 저장합니다.
create table if not exists public.soop_viewer_samples (
  id bigint generated by default as identity primary key,
  streamer_id varchar(100) not null,
  broad_no varchar(100),
  viewers integer not null check (viewers >= 0),
  sampled_at timestamptz not null default now()
);

create index if not exists soop_viewer_samples_streamer_time_idx
on public.soop_viewer_samples(streamer_id, sampled_at desc);

-- 같은 방송의 같은 분 샘플이 중복 저장되지 않도록 합니다.
create unique index if not exists soop_viewer_samples_minute_unique_idx
on public.soop_viewer_samples(streamer_id, broad_no, sampled_at);

alter table public.soop_viewer_samples enable row level security;

drop policy if exists "soop_viewer_samples_public_read" on public.soop_viewer_samples;
drop policy if exists "soop_viewer_samples_public_insert" on public.soop_viewer_samples;

create policy "soop_viewer_samples_public_read"
on public.soop_viewer_samples for select
to anon, authenticated
using (streamer_id = 'bboringirl');

-- 방문자가 사이트를 열어 둔 동안 브라우저가 시청자 샘플을 저장할 수 있도록 합니다.
-- publishable/anon key는 브라우저에 노출되므로 이 방식에서는 임의의 값 입력을 완전히 막을 수 없습니다.
create policy "soop_viewer_samples_public_insert"
on public.soop_viewer_samples for insert
to anon, authenticated
with check (
  streamer_id = 'bboringirl'
  and viewers >= 0
  and viewers <= 100000000
);
